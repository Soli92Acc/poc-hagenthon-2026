#!/usr/bin/env bash
# =============================================================================
# compute-human-cost.sh — deterministic tool compute_human_cost (EP-009, US-034, TSK-062)
# =============================================================================
#
...
# no LLM). It is an agnostic READER of the `analytics/events/` side-channel (written
# single-source from record_task_event, US-033) + a rate card READER
# versioned `analytics/rates.yaml`. The tool does NOT reason: it filters events
# `actor_type: human`, resolve `actor_id → role_id`, apply cost formula
# verbatim human, enforces privacy N>=5 and outputs pure JSON.
# Narrative interpretation is the scope of the skill (US-036), never of this tool.
#
# PATTERN.md §3 — optional canonical operation «Cost Computation» (Human Cost),
#   with the TWO operational invariants of the capability:
#     - minimum aggregation N>=5 on personal data (ADR-023 §C).
#     - explicit rate_basis in every output that uses the rate card (ADR-023 §E).
# Wiki: wiki/concepts/task-analytics-cost-estimation-capability.md
#       [[task-analytics-cost-estimation-capability]] §Human cost §Limitations.
#
# FORMULA verbatim (concept §2.2):
#       costo_umano(task) = ore_sforzo · tariffa_ruolo      # da <<rate_card>>
#   effort_hours: from `effort_hours` (TSK frontmatter / event extras, override
#       explicit) or auto-derived from ts(finished) - ts(started).

#   rate_role: from analytics/rates.yaml, checked for valid `valid_from`
#       to the effort timestamp. NEVER hardcode your code.
#
# ADR-022 — schema `<<rate_card>>` (analytics/rates.yaml): git versioned YAML with
#   `valid_from` semi-aperto a destra (§E), canonical role `id` + `aliases:` (§C),
#   `rate_basis` at mandatory file level (§C), resolution role/valid_from (§D).
# ADR-023 — analytics tool registration in `.claude/tools/analytics/*` (no MCP),

#   minimal contract: --config optional, stdout=JSON / stderr=log, exit code
#   semantic, stateless (§A). Data Policy `<<data_policy>>`:
#     §C — minimum aggregation N>=5: `executive`/`project` report with N < threshold
#          distinct actor_id ⇒ `per_actor` OMITTED (never individual cost exposed),
#          replaced by aggregation by role + suppression note.
...
#          fallback `role: unknown` + warning (no fail-loud, recoverable event).
#     §E — explicit `rate_basis` + fail-loud on mismatch config vs file.

#
# -----------------------------------------------------------------------------
# INPUT CONTRACT (CLI)
#   --task-id <id> parses a single task (mutually excl. with --filter).
#   --filter '<JSON>' filter on event store, default {} (global aggregate).

#                         Shape: {project_id?, task_type?, period?:{from,to}}.

#   --rates <path> rate card, default analytics/rates.yaml (or

#                         analytics.measurement.rate_card_path da config).
#   --audience <lvl> executive | project | operational (operational default).

#                         Privacy masking governs N>=5 (ADR-023 §C).

#   --rate-as-of <date> what-if: force resolution valid_from to a date

#                         ISO-8601 specifica (proiezione esplicita, ADR-022 §F).
#   --config <path> default "factory.config.yaml" (ADR-023 §A).

#
# OUTPUT CONTRACT (stdout, pure JSON) — scheme US-034 / ADR-023 §E:
#   {
#     "cost": <float>, "currency": "EUR",
#     "rate_basis": "fully-loaded|bill-rate",          // se cost > 0
#     "breakdown": {
#       "per_role":  {"<role_id>": <float>, ...},
#       "per_actor": {"<actor_id>": <float>, ...} // OMITTED if N < threshold

#     },
#     "events_considered": <int>,
#     "rate_card_version": "<git-hash|date>",
#     "notes": [ ... ] // include privacy note if suppressed
#   }
#
# STDERR
#   human-readable log (non-blocking warnings; fail-loud on error).


#
# EXIT CODES
#   0 cost calculated OR no-op (analytics.measurement.enabled absent/false, R.P3)
#   >0 error (prerequisite missing, rates absent/unreadable at capability ON,
#      mismatch rate_basis, invalid filter/arguments).
...
# --task-id and --filter are mutually exclusive.
...
# Validate --filter JSON well-formed (fail-loud).
...
# Valid --rate-as-of if present (ISO-8601 date YYYY-MM-DD).
...
# ---------------------------------------------------------------------------
# 3. Minimal YAML helper (no yq) — extracts a scalar from analytics.measurement.<key>.

#    Same parser as record-event.sh / analyze-timeline.sh for exact consistency.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 4. Master switch — no-op if the capability is turned off (R.P3, ADR-023 §I)
...
# Config values ​​(with default ADR-021/022/023).
...
# rate_card_path: --rates takes precedence, then config, then default.
...
# rate_basis declared in config (for cross-file validation, ADR-023 §E).
CFG_RATE_BASIS="$(yaml_measurement_value "rate_basis" "$CONFIG")"

# ---------------------------------------------------------------------------
# 5. Rate card — fail-loud if absent at capability ON (ADR-022 §G, R.P3).
#    NB: the no-op lives ONLY on the master switch (§4). Here the capability is ON:
#    the absence of the rate card is an actionable error, not a no-op.
# ---------------------------------------------------------------------------
...
#     Extracts: currency, rate_basis (file-level), roles[] with id, aliases[], rates[].
RATES_JSON="$(awk '
  ...
  END {
    # Outputs JSON via jq @json on values; here we build a raw structure.
    printf "{"
...
# Validates that the parse is well-formed (defensive) JSON.
...
# 5.2 rate_basis at file level REQUIRED (ADR-022 §C).
...
#     Fail-loud on mismatch (better noisy error than ambiguous report).
...
# 5.4 rate_card_version: git hash of the file if in git repo, otherwise date of the file.
...
if [[ "$RATE_CARD_VERSION" == "unknown" ]]; then
  # Fallback: file mtime (YYYY-MM-DD), deterministic for audit.
...
parse_actor_map_yaml() {
  # Extracts "key: value" pairs under the 'actors:' key (actors.yaml file) or
  # under 'actors_map:' nested in analytics.measurement (factory.config.yaml).
...
# 6a. Priority 1: analytics/actors.yaml (gitignored opt-in).
...
# 6b. Priority 2 (fallback): factory.config.yaml analytics.measurement.actors_map.
#     Only for keys not already resolved by the file (file takes precedence).
...
# ---------------------------------------------------------------------------
# 7. load_events(filter) — reader agnostic (ADR-021 §D), actor_type=human only.
#    If --task-id is given, also filter by task_id. Same dispatch as
#    analyze-timeline.sh for exact consistency.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 8. Human cost calculation — a single deterministic jq program.
#    For each task (group_by task_id):
#      - actor_id = actor_id of the task (first event).
#      - role_id   = actors_map[actor_id] // "unknown" (warning lato shell).
#      - effort_hours = explicit effort_hours (extras.effort_hours, last non-null)
...
#                     where as_of = --rate-as-of if given, otherwise the ts effort.

#      - cost = effort_hours · role_rate (formula §2.2 verbatim).
...
COMPUTE="$(printf '%s\n' "$EVENTS" | jq -s \
  ...
  # ISO-8601 → epoch seconds (UTC). Same logic as analyze-timeline.sh.
  def epoch:
    sub("\\.[0-9]+";"") as $s0
  ...
  # Resolve hourly_rate for role_id to a given ISO ($asof, YYYY-MM-DD...).
  # Match canonical id OR alias; poi valid_from semi-aperto (ADR-022 §D/§E).
  # Returns {rate, found, canonical_role} — found=false if role or date not
  # solvable. canonical_role = canonical id of the rate card (normalized alias
  # to canonical, ADR-022 §C/§D); fallback to the passed $role if not found.
  ...
  | ([ $by_task[]
       ...
       # explicit effort_hours (last non-null among task events).

       | ([ $evs[] | .extras.effort_hours // empty ] | last) as $eff_override
       # ts of the first started and last finished (by derivation effort + as_of).
       ...
       # as_of for rate resolution: --rate-as-of (projection) or ts effort.
  ...
  # Warning collectors for the shell.
...
# ---------------------------------------------------------------------------
# 9. Non-blocking warnings on stderr (ADR-023 §D — unmapped actor is recoverable).
...
# ---------------------------------------------------------------------------
# 10. Privacy masking N>=5 (ADR-023 §C) — NON-NEGOTIABLE INVARIANT.
#     Decisione di soppressione di per_actor:
#       - executive | project : N < MIN_N ⇒ SUPPRESS per_actor (never exceptions).
#       - operativa            : SOPPRIMI se operational_show_actor_id=false;
#                                otherwise it shows MA with warning below threshold.
#     "Suppression" = the per_actor field is OMITTED from the output: no cost
#     individual is never exposed below the threshold. per_role always remains the aggregate.
# ---------------------------------------------------------------------------
...
printf '%s' "$COMPUTE" | jq \
  ...
  # rate_basis REQUIRED only if cost > 0 (DoD US-034 + ADR-023 §E).
  | ( if $cost > 0 then . + { rate_basis: $rate_basis } else . end )
  # notes: privacy (if present) + threshold + rate_basis (if cost>0).
  | . + { notes: ( []
