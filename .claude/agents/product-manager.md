...
In contesti multi-utente, prima di scrivere in `management/kanban/`:

1. Leggi `memory/active-sessions.yaml`. Se assente → no-op.
2. Rimuovi entry scadute. Controlla clash su domain `kanban`. Se clash → WARN.
3. Aggiungi claim `{uuid, agent: product-manager, domain: kanban, started_at, ttl_min: 30}`.
