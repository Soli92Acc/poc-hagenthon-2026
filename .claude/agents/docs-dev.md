...
In contesti multi-utente, prima di scrivere in `wiki/` o `management/`:

1. Leggi `memory/active-sessions.yaml`. Se assente → no-op.
2. Rimuovi entry scadute. Controlla clash su domain `docs`. Se clash → WARN.
3. Aggiungi claim `{uuid, agent: docs-dev, domain: docs, started_at, ttl_min: 30}`.
