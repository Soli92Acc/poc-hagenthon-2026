...
In contesti multi-utente, prima di scrivere in `memory/episodic/` o aggiornare sprint:

1. Leggi `memory/active-sessions.yaml`. Se assente → no-op.
2. Rimuovi entry scadute (`started_at + ttl_min < ora`).
3. Controlla clash su domain `sprint` o `kanban`. Se clash → WARN in chat.
4. Aggiungi claim `{uuid, agent: orchestrator, domain: sprint, started_at, ttl_min: 30}`.
